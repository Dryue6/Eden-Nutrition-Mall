...
const AddressList: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editForm.id) {
        await addressApi.update(editForm);
      } else {
        await addressApi.add(editForm);
      }
      setIsEditing(false);
      fetchAddresses();
    } catch (error) {
    ...
    return (
      <div className="bg-gray-50 min-h-screen pb-24">
        <header className="bg-white p-4 sticky top-0 z-10 flex items-center gap-4 shadow-sm">
          <button onClick={() => setIsEditing(false)}><ChevronLeft size={24} /></button>
          <h1 className="text-lg font-bold text-gray-800">{editForm.id ? '编辑地址' : '新增地址'}</h1>
        </header>
        <form onSubmit={handleSave} className="p-4 space-y-4">
          ...
          <button type="submit" className="w-full bg-emerald-600 text-white py-3.5 rounded-full font-bold shadow-lg mt-8">
            保存地址
          </button>
        </form>
      </div>
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-24">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                ...
                >
                  <div className={cn(
                    ...
                    {addr.isDefault === 1 && <Check size={10} className="text-white" />}
                  </div>
                ...
                <div className="flex gap-4">
                  <button onClick={() => openForm(addr)} className="text-xs text-gray-400">编辑</button>
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-4 bg-white border-t border-gray-100 z-50">
        <button onClick={() => openForm()} className="w-full bg-emerald-600 text-white py-3.5 rounded-full font-bold shadow-lg shadow-emerald-100 flex items-center justify-center gap-2">
          <Plus size={20} />
          添加新地址
        </button>
      </div>
